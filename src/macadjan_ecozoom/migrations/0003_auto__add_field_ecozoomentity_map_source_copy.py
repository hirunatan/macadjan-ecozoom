...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'EcozoomEntity.map_source_copy'
        db.add_column(u'macadjan_ecozoom_ecozoomentity', 'map_source_copy',
                      self.gf('django.db.models.fields.related.ForeignKey')(related_name='entities2', null=True, on_delete=models.PROTECT, to=orm['macadjan_ecozoom.MapSource']),
                      keep_default=False)

        if not db.dry_run:
            for entity in orm.EcozoomEntity.objects.all():
                entity.map_source_copy_id = entity.map_source_id
                entity.save()


    def backwards(self, orm):
        # Deleting field 'EcozoomEntity.map_source_copy'
        db.delete_column(u'macadjan_ecozoom_ecozoomentity', 'map_source_copy_id')
    ...
    models = {
        ...
        u'macadjan_ecozoom.ecozoomentity': {
            ...
            'map_source_copy': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'entities2'", 'null': 'True', 'on_delete': 'models.PROTECT', 'to': u"orm['macadjan_ecozoom.MapSource']"}),
