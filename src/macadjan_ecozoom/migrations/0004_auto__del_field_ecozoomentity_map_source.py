...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        if not db.dry_run:
            for source in orm['macadjan_ecozoom.MapSource'].objects.all().order_by('id'):  # It's important to maintain the same ids in new table
                new_source = orm['macadjan.MapSource'].objects.create(
                    id = source.id,
                    name = source.name,
                    slug = source.slug,
                ...
                if source.id != new_source.id:
                    raise ValueError('Houston, we have a problem, this should not occur')


    def backwards(self, orm):
        # Adding field 'EcozoomEntity.map_source'
        db.add_column(u'macadjan_ecozoom_ecozoomentity', 'map_source',
                      self.gf('django.db.models.fields.related.ForeignKey')(related_name='entities', null=True, on_delete=models.PROTECT, to=orm['macadjan_ecozoom.MapSource']),
                      keep_default=False)
